...
for(ss in 1:Simu){
  ...
  all_row_combinations_sum <- function(m){
    idx <- expand.grid(rep(list(seq_len(ncol(m))), nrow(m)))
    rowSums(sapply(seq_len(nrow(m)), function(i) m[i, idx[[i]]]))
  }
  
  logps = function(u,n,Ti,C,Beta,Betas,Gamma,Gammas,Delta,block,k){
    row_block = nrow(block)
    col_block = ncol(block)
    fin1 = fin2 = rep(NA, row_block)
    fin3 = matrix(NA, row_block, Ti-2)
    for(t in 1:Ti){
      grid = expand.grid(rep(list(1:k), col_block))
      grid_mat <- as.matrix(grid)
      for(j in 1:row_block){
        block_idx = block[j, ]
        den = 0
        num = 0
        
        #fun
        potential = function(block_state){
          
          #ag
          u_ad_t = u[, t]
          u_ad_t[block_idx] = block_state
          
          #Ma
          Ma_block = C[block_idx, ]*matrix(u_ad_t, nrow = length(block_idx), ncol = n, byrow = TRUE)
          
          spatial_term =
            sum(table(factor(rep(block_state, n), 1:k),
                      factor(Ma_block, 1:k)) *
                  if(t==1) Gamma else Gammas)
          if(t == 1){
            idx = matrix(c(block_state, u[block_idx, t+1]), ncol = 2)
            temporal_term = sum(Delta[idx])
            exp(sum(Beta[block_state]) +
                  spatial_term +
                  temporal_term)
            
          }else if(t == Ti){
            idx = matrix(c(u[block_idx, t-1],
                           block_state), ncol = 2)
            temporal_term = sum(Delta[idx])
            exp(sum(Betas[block_state]) +
                  spatial_term +
                  temporal_term)
          }else{
            idx_prev = matrix(c(u[block_idx, t-1], block_state), ncol = 2)
            idx_next = matrix(c(block_state, u[block_idx, t+1]), ncol = 2)
        ...
        #Den
        den = sum(apply(grid_mat, 1, potential))
        
        #Num
        current_state = u[block_idx, t]
        num = potential(current_state)
    ...
    sum(log(fin1)) + sum(log(fin2)) + sum(log(c(fin3)))
  }
  
  balanced_kmeans = function(Com, k){
    U = Com
    n = nrow(U)
    target_size = floor(n/k)
    
    #Init k-means
    km = kmeans(U, centers = k, nstart = 20)
    centers = km$centers
    
    #Comp dist matrix
    dist_matrix = as.matrix(dist(rbind(centers, U)))[1:k, (k+1):(k+n)]
    
    #As with balance constraint
    clusters = rep(NA, n)
    cluster_sizes = rep(0, k)
    
    order_nodes = order(apply(dist_matrix, 2, min))
    
    for(i in order_nodes){
      dists = dist_matrix[, i]
      sorted_clusters = order(dists)
      for(c in sorted_clusters){
        if(cluster_sizes[c] < target_size){
          clusters[i] = c
  ...
  spectral_clustering = function(A, k, balanced = FALSE){
    n = nrow(A)
    eps = 1e-10
    
    #Normalized Laplacian: L_sym = I - D^{-1/2} A D^{-1/2}
    D_inv_sqrt = diag(1/sqrt(rowSums(A) + eps))
    ...
    U = eig$vectors[, order(eig$values)[1:k]]
    
    #Row normalization
    U = U/sqrt(rowSums(U^2) + eps)
    
    #Clustering
    if (!balanced) {
      clusters = kmeans(U, centers = k, nstart = 20)$cluster
    }else{
      clusters = balanced_kmeans(U, k)
    }
    return(clusters)
  }
  ...
  clusters = spectral_clustering(Com, k = num_block, balanced = TRUE)
  block = matrix(NA, num_block, n/num_block)
  print(clusters)
  for(i in 1:num_block){
    block[i,] = which(clusters == i)
  }
  ...
  #Parameters
  Gamma_ = 2*matrix(c(0,-1/2,-1/2,-1/2,0,-1/2,-1/2,-1/2,0), 3,3)
  Delta_ = matrix(c(0,-1,-1,-1,0,-1,-1,-1,0), 3,3)
  Beta_ = c(0,0,0)
  Gammas_ = 2*matrix(c(0,-1/2,-1/2,-1/2,0,-1/2,-1/2,-1/2,0), 3,3)
  Betas_ = c(0,0,0)
  ...
  for(r in 2:R){
    ...
    for(j in 1:(k-1)){
      ...
      #Pseudo beta
      ratio_b = log_prior_theta(Betan[j], mb, sb_b) - log_prior_theta(Beta[j], mb, sb_b) + logps(u,n,Ti,C,Betan,Betas,Gamma,Gammas,Delta,block,k) - logps(u,n,Ti,C,Beta,Betas,Gamma,Gammas,Delta,block,k)
    ...
    for(j in 1:(k-1)){
      ...
      #Pseudo betas
      ratio_b = log_prior_theta(Betasn[j], mb, sb_b) - log_prior_theta(Betas[j], mb, sb_b) + logps(u,n,Ti,C,Beta,Betasn,Gamma,Gammas,Delta,block,k) - logps(u,n,Ti,C,Beta,Betas,Gamma,Gammas,Delta,block,k)
    ...
    for(j in 1:(k-1)){
      for(l in (j+1):k){
        ...
        ratio_g = log_prior_theta(Gamman[j,l], mb, sb) - log_prior_theta(Gamma[j,l], mb, sb) + logps(u,n,Ti,C,Beta,Betas,Gamman,Gammas,Delta,block,k) - logps(u,n,Ti,C,Beta,Betas,Gamma,Gammas,Delta,block,k)
    ...
    for(j in 1:(k-1)){
      for(l in (j+1):k){
        ...
        ratio_g = log_prior_theta(Gammasn[j,l], mb, sb) - log_prior_theta(Gammas[j,l], mb, sb) + logps(u,n,Ti,C,Beta,betas,Gamma,Gammasn,Delta,block,k) - logps(u,n,Ti,C,Beta,betas,Gamma,Gammas,Delta,block,k)
    ...
    for(j in 1:(k-1)){
      for(l in (j+1):k){
        ...
        ratio_d = log_prior_theta(Deltan[j,l], mb, sb) - log_prior_theta(Delta[j,l], mb, sb) + logps(u,n,Ti,C,Beta,betas,Gamma,Gammas,Deltan,block,k) - logps(u,n,Ti,C,Beta,betas,Gamma,Gammas,Delta,block,k)
  ...
  #setwd("")
  save.image(paste0("Sim_Gnm_k3_n40_t5_sigmasqrt1_rv_pseudo_block_adj_",ss,".Rdata"))
  rm(list = ls())
}
